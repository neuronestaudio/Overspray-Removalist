/* Render build/sop.html to the SSG+SPA build SOP PDF.
   Run after build/sop.py:  node build/sop.js

   Two passes, merged afterwards: the cover bleeds to the paper edge at zero
   margin, the body carries page margins and a running footer. Chromium applies
   one margin box per document, so a single pass cannot do both. */
const { chromium } = require('playwright');
const path = require('path');
const fs = require('fs');

const SRC = 'file:///' + path.resolve(__dirname, 'sop.html').replace(/\\/g, '/');
const COVER = path.resolve(__dirname, '.sop-cover.pdf');
const BODY = path.resolve(__dirname, '.sop-body.pdf');
  ...
  await b.pdf({
    ...
    footerTemplate: `
      <div style="width:100%;font-family:Arial,sans-serif;font-size:7pt;color:#8b93a7;
        ...
        <span>Pendulum Digital &nbsp;&middot;&nbsp; build standard</span>

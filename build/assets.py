...
# Emit the variant map the app imports, so a component can never request a width
# that was never generated. Sources here vary from 540px to 2070px wide and
# nothing is upscaled, so "every image has a 1200 variant" is simply not true and
# hardcoding widths in JSX produces silent 404s.
APP_DATA = os.path.join(ROOT, "app", "src", "data")
if os.path.isdir(os.path.dirname(APP_DATA)):
    os.makedirs(APP_DATA, exist_ok=True)
    entries = []
    for stem in sorted(manifest):
        variants = manifest[stem]
        w, h = variants[-1][1], variants[-1][2]
        widths = ", ".join(str(v[1]) for v in variants)
        entries.append(
            f"  '{stem}': {{ widths: [{widths}], width: {w}, height: {h} }},"
        )
    ts = (
        "/**\n"
    ...
    with open(os.path.join(APP_DATA, "images.ts"), "w", encoding="utf-8") as f:
        f.write(ts)
    print(f"manifest: app/src/data/images.ts ({len(manifest)} stems)")
